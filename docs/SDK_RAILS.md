...
These rails are exposed in `@receiz/sdk@108.0.0`. The app treats customer accounts as tenant-scoped storefront projections over Receiz proof. The same proof-bearing Receiz identity can be used across multiple stores, but orders, rewards, assets, and permissions are projected for the active subdomain or custom domain. SDK `doctor()` reports delegated-token, tenant, customer, merchant, commerce, media, domain, and public-store requirements directly.
...
With `@receiz/sdk@108.0.0`, typed app-state, signed public-store publish, Twin, World, commerce runtime, artifact custody, native Record → Seal, verified extraction, same-account profile updates, cross-platform bearer ownership, causal replay, and deterministic checkpoints are exposed through the browser-safe universal runtime. Node-only compiler APIs remain at `@receiz/sdk/compiler`; deterministic emulator and conformance APIs live at `@receiz/sdk/testing`; historical v107 wire contracts are isolated behind `@receiz/sdk/v107`. `receiz.constitution.json` is validated at `src/lib/receiz/constitution.ts` as an app overlay chained to the canonical v108 registry digest used by MCP and every v108 AI-skill manifest. Complete artifact bytes are verified before payload extraction, and ownership changes preserve immutable object identity, payload, provenance root, prior history, and unknown namespaces. Existing sealed proof and verified local history stay stronger than all projections. `Publish theme` adopts the same authoritative public-store revision; browser storage remains only an offline workspace projection and cross-tab synchronization mechanism.
