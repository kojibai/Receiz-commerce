...
These rails are exposed in `@receiz/sdk@106.0.0`. The app treats customer accounts as tenant-scoped storefront projections over Receiz proof. The same proof-bearing Receiz identity can be used across multiple stores, but orders, rewards, assets, and permissions are projected for the active subdomain or custom domain. SDK `doctor()` reports delegated-token, tenant, customer, merchant, commerce, media, domain, and public-store requirements directly.
...
With `@receiz/sdk@106.0.0`, typed app-state, signed public-store publish, Twin, World, commerce runtime, domain, customer, merchant, media, portability, release, constitution, command-admission, capability-authority, atomic-store, and causal-history namespaces are exposed through the browser-safe universal runtime. Node-only compiler and migration APIs remain at `@receiz/sdk/compiler`; deterministic emulator and conformance APIs live at `@receiz/sdk/testing`. `receiz.constitution.json` is validated at `src/lib/receiz/constitution.ts`; its digest must remain identical to the SDK release identity, MCP context, and every v106 AI-skill manifest. New canonical global writes must enter through scoped capabilities, expected revisions, causal parents, stable idempotency keys, executable law evaluation, atomic receipt-producing admission, and replayable causal records. Existing sealed proof and verified local history stay stronger than all projections. `Publish theme` adopts the same authoritative public-store revision; browser storage remains only an offline workspace projection and cross-tab synchronization mechanism.
