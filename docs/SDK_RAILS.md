...
These rails are exposed in `@receiz/sdk@112.0.0`. The app treats customer accounts as tenant-scoped storefront projections over Receiz proof. The same proof-bearing Receiz identity can be used across multiple stores, but orders, rewards, assets, and permissions are projected for the active subdomain or custom domain. SDK `doctor()` reports delegated-token, tenant, customer, merchant, commerce, media, domain, and public-store requirements directly.
...
With `@receiz/sdk@112.0.0`, typed app-state, signed public-store publish, Twin, World, commerce runtime, artifact custody, native Record → Seal, typed local verification, profile admission, transition planning, capability signing, durable staging, named-domain commit, projection-only public proof, causal replay, and deterministic checkpoints are exposed through the browser-safe universal runtime. Node-only compiler APIs remain at `@receiz/sdk/compiler`; deterministic emulator and conformance APIs live at `@receiz/sdk/testing`. `receiz.constitution.json` is validated at `src/lib/receiz/constitution.ts` as an app overlay chained to the package-embedded canonical v112 registry digest used by MCP and every v112 AI-skill manifest. Exact bytes must be reverified at process boundaries; runtime admissions, plans, capabilities, candidates, stores, staged references, and receipts cannot be serialized into authority. Existing sealed proof and accepted artifact history stay stronger than all projections. `Publish theme` adopts the same authoritative public-store revision; browser storage remains only an offline workspace projection and cross-tab synchronization mechanism.
