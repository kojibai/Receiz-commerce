...
These rails are exposed in `@receiz/sdk@107.0.0`. The app treats customer accounts as tenant-scoped storefront projections over Receiz proof. The same proof-bearing Receiz identity can be used across multiple stores, but orders, rewards, assets, and permissions are projected for the active subdomain or custom domain. SDK `doctor()` reports delegated-token, tenant, customer, merchant, commerce, media, domain, and public-store requirements directly.
...
With `@receiz/sdk@107.0.0`, typed app-state, signed public-store publish, Twin, World, commerce runtime, domain, customer, merchant, media, portability, release, constitution, command-admission, capability-authority, atomic-store, causal-history, identity/profile, portable-continuity, bearer-ownership, proof-head, receipt, and offline-command namespaces are exposed through the browser-safe universal runtime. Node-only compiler and migration APIs remain at `@receiz/sdk/compiler`; deterministic emulator and conformance APIs live at `@receiz/sdk/testing`. `receiz.constitution.json` is validated at `src/lib/receiz/constitution.ts`; its digest must remain identical to the SDK release identity, MCP context, and every v107 AI-skill manifest. New canonical global writes must enter through scoped capabilities, expected revisions, causal parents, stable idempotency keys, executable law evaluation, atomic receipt-producing admission, and replayable causal records. Queued offline operations remain signed proposals with `globallyCommitted: false` until a canonical receipt verifies. Existing sealed proof and verified local history stay stronger than all projections. `Publish theme` adopts the same authoritative public-store revision; browser storage remains only an offline workspace projection and cross-tab synchronization mechanism.
