...
describe("Receiz v107 application contract", () => {
  it("compiles the complete app contract through the v107 SDK", async () => {
    ...
    const plan = compiler.compileReceizAppContract(contract, { targetSdkVersion: "107.0.0" });
  ...
  it("passes the v107 integration check with verified production rails", async () => {
    const compiler = await import("@receiz/sdk/compiler");
