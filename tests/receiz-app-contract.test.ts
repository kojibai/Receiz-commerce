...
describe("Receiz v116 application contract", () => {
  it("compiles the complete app contract through the v116 SDK", async () => {
    ...
    const plan = compiler.compileReceizAppContract(contract, { targetSdkVersion: "116.0.0" });
    ...
    assert.deepEqual(contract.operations, compiler.RECEIZ_V116_APPLICATION_OPERATION_MATRIX);
    assert.equal(contract.operations.length, 16);
    assert.equal(compiler.RECEIZ_CURRENT_APPLICATION_OPERATION_MATRIX_DIGEST, "ec5829eeec039c1f4885d056b8cd6cf6506d08547cee58daa229ecbd44155420");
    assert.ok(plan.verificationCommands.length > 0);
  });

  it("passes the v116 integration check with verified production rails", async () => {
    const compiler = await import("@receiz/sdk/compiler");
