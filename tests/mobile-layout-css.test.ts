...
describe("mobile storefront layout CSS", () => {
  ...
  it("reserves the compact world-status rail so it never covers the player identity", () => {
    assert.match(css, /\.mobile-play-wrap \.wilds-living-world-hud \{[^}]*right: 8px;[^}]*bottom: 86px;[^}]*flex-direction: column;[^}]*align-items: flex-end;/s);
    ...
    assert.match(css, /\.mobile-play-wrap \.wilds-living-world-hud \{[^}]*flex-direction: column;[^}]*align-items: flex-end/s);
