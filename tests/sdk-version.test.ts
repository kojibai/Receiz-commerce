...
describe("Receiz v111 dependency contract", () => {
  it("pins the supported v111 SDK, MCP, AI skills, registry, and vendored publication bridge", () => {
    ...
    assert.deepEqual(RECEIZ_V111_ARTIFACT_LAWS, Array.from({ length: 20 }, (_, index) => `ARTIFACT-${String(index + 1).padStart(3, "0")}`));
    assert.equal(pkg.dependencies["@receiz/sdk"], "111.0.0");
    assert.equal(pkg.dependencies["@receiz/mcp-server"], "111.0.0");
    assert.equal(pkg.dependencies["@receiz/ai-skills"], "111.0.0");
    assert.equal(pkg.scripts?.["receiz:check"], "receiz app check --target 111.0.0 --json");
    ...
    assert.equal(pkg.pnpm?.overrides?.["@receiz/sdk"], "file:vendor/receiz-sdk-111.0.0.tgz");
    assert.equal(pkg.pnpm?.overrides?.["@receiz/mcp-server"], "file:vendor/receiz-mcp-server-111.0.0.tgz");
    assert.equal(pkg.pnpm?.overrides?.["@receiz/ai-skills"], "file:vendor/receiz-ai-skills-111.0.0.tgz");
  });
  ...
  it("documents the supported MCP pair and authoritative theme publication", () => {
    ...
    assert.match(readme, /@receiz\/mcp-server@111\.0\.0/);
    assert.match(readme, /@receiz\/ai-skills@111\.0\.0/);
    assert.match(readme, /Publish theme/);
    assert.match(rails, /@receiz\/sdk@111\.0\.0/);
  ...
  it("keeps the v111 Node-only compiler outside browser bundles", () => {
  ...
  it("exposes canonical v111 artifact-derived authority, admission, recovery, causal history, capabilities, errors, emulator, and zero-network conformance", async () => {
    ...
    assert.equal(descriptor.packageCompatibility.sdk, ">=111.0.0 <112.0.0");

    const report = await runReceizConformance();
    assert.equal(report.schema, "receiz.sdk.conformance_report.v1");
    assert.equal(report.sdkVersion, "111.0.0");
