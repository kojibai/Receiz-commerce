...
describe("Receiz v124.0.3 coordinated release identity", () => {
  ...
  it("ships the complete 165-tool server and all 22 v124 runtime tools", () => {
    const names = new Set(RECEIZ_MCP_TOOLS.map((tool) => tool.name));
    assert.equal(RECEIZ_MCP_TOOLS.length, 165);
