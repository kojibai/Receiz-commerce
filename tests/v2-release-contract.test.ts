...
test("the repository identifies the shipped release as 4.4.0", async () => {
  ...
  assert.equal(packageJson.scripts?.["receiz:check"], "receiz app check --target 111.0.0 --json");
  assert.match(lockfile, /'@receiz\/sdk':[\s\S]*?version: file:vendor\/receiz-sdk-111\.0\.0\.tgz/);
  assert.match(lockfile, /'@receiz\/mcp-server':[\s\S]*?version: file:vendor\/receiz-mcp-server-111\.0\.0\.tgz/);
  assert.match(lockfile, /'@receiz\/ai-skills':[\s\S]*?version: file:vendor\/receiz-ai-skills-111\.0\.0\.tgz/);
});
