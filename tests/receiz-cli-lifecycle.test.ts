...
const verifier = join(root, "scripts", "receiz-v127-migration-verify.mjs");
...
describe("Receiz v124 CLI lifecycle", () => {
  it("verifies the audited proof-preserving application upgrade", () => {
    ...
    assert.equal(report.schema, "receiz.repository.v127-migration-verification.v1");
  ...
  it("routes public package commands through the end-to-end CLI checks", () => {
    ...
    assert.equal(
      ...
      "node scripts/receiz-v127-migration-verify.mjs --root ."
    );
    ...
    assert.match(cliCheck, /receiz-v127-inspection\.mjs/);
    assert.match(cliCheck, /receiz-v127-migration-verify\.mjs/);
