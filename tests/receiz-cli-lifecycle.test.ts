...
const verifier = join(root, "scripts", "receiz-v111-migration-verify.mjs");

describe("Receiz v111 CLI lifecycle", () => {
  it("verifies the audited proof-preserving application upgrade", () => {
    ...
    assert.equal(report.schema, "receiz.repository.v110-v111.migration-verification.v1");
  ...
  it("routes public package commands through the end-to-end CLI checks", () => {
    ...
    assert.equal(
      ...
      "node scripts/receiz-v111-migration-verify.mjs --root ."
    );
