...
Current release: `5.3.0` · SDK target: `@receiz/sdk@127.0.0` · ruleset: `127.0.0`
...
The v127 Receiz toolchain is pinned exactly as `@receiz/sdk@127.0.0`, `@receiz/mcp-server@127.0.0`, and `@receiz/ai-skills@127.0.0` from the public npm registry. The lockfile pins their published integrity values with no Receiz package override. The local 43-skill `ai-skills/` tree is byte-aligned with the published doctrine, excluding only the npm package manifest, and can be checked with `pnpm validate:ai-skills`.

`receiz.app.json` declares the complete 60-operation v127 authority matrix, while `receiz.constitution.json` is the app law overlay chained to the package-embedded canonical v127 registry digest. The MCP server exposes 330 tools, including all 22 v124 production-runtime outcomes plus verified material opening and sealed-pulse Kai projection; the AI package exposes 43 skills, 37 manifests, and 34 agent prompts. The package release and constitutional ruleset are both `127.0.0`; the app verifies both independently. JSON cannot mint session, plan, handle, private-result, replay-candidate, or sealed-source custody. Run `pnpm receiz:check`, `pnpm receiz:conformance`, `pnpm receiz:authority-scan`, and `pnpm receiz:release-lock` for the complete evidence chain.
...
`pnpm release:check` runs the tracked-file secret scan, tests, typecheck, the v127 app contract and 60-operation matrix check, general SDK conformance, retained living-subject conformance, v127 migration and retained runtime authority checks, the release lock, AI-skill validation, lint, a guarded production build, and Receiz doctor. Public forks can run doctor without static access tokens. Production readiness comes from `runtime.qualifyV124` dependency evidence—not method presence.
...
The local command resolves the pnpm-pinned `@receiz/mcp-server@127.0.0`. Use the matching `@receiz/ai-skills@127.0.0` doctrine from `node_modules/@receiz/ai-skills` or the aligned local `ai-skills/` directory. V124 retains every inherited surface and adds 22 production-runtime tools for Kai, proof challenges, authority-session lifecycle, atomic stage/execute/recovery/cancel, operational qualification, verified public/private replay and checkpoints, sealed replay portability, exact namespaces, privacy-safe recipients, and sealed-source publication. V124.0.3 introduced `receiz_material_url_open` and `receiz_sealed_kai_moment`; the app verifies and plays `rma2` or `rmc1` material at `/verify#material=…`, exposes the canonical proof link, and never lets transport or presentation outrank the sealed object. The retained runtime mapping is at `/developers/receiz`.
...
See [v5.3.0 / v127 release notes](docs/releases/2026-09-23-v127-full-release.md) for the complete changes and verification. V127 installs offline sealing, Kai proving, and local subject-host entrypoints. These require explicit custody configuration; this app does not enroll devices automatically. Historical Connect calls require an explicit compatibility host and are unavailable on the default canonical host. The release preserves this fail-closed behavior and tests it.
