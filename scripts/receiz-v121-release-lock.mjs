...
const EXPECTED_RANGE = ">=121.0.0 <122.0.0";
const EXPECTED_REGISTRY = "29a793a5bcc0195ab41d30614d37ac51df66023af354fa4335460764eb0af413";
const EXPECTED_MATRIX = "208553829ba78a5536524b864577ce59989e2d0a994fad9598d39ae3d557c4f5";
const EXPECTED_REDUCER = "5694662e2acc8b886ac9697ffad202b411d7e66c5f26f9106ee0768df7c7b8c8";
const EXPECTED_INTEGRITIES = {
  "@receiz/sdk": "sha512-alHaHXyAGl+mcN5ZuqbldgmF19QZBsq71hsmITsh8OifutsnEEzlIYoXUkxHtiNT1QE7rL7EEB2CWNjYU7yGXQ==",
  "@receiz/mcp-server": "sha512-8EOnvUNglZgHAc1rM0DIa4aFOSdVTMDcspf9xwbeKpXjnc+gMkF2/UK9P6pKm80yl+j+DWT5WAxJICeu77g3zg==",
  "@receiz/ai-skills": "sha512-wVCHfb9oLmSHJW2+pFUqxe5dDdDeRErbhEmcqz8q7SVSQBommRA3E1K73zDLHeGikWMnkmfZr+51X/feB1HtZQ==",
};
...
check("registry:canonical-v121-chain", registry.previousRegistryDigest === RECEIZ_V121_REGISTRY_DIGEST && RECEIZ_V121_REGISTRY_DIGEST === EXPECTED_REGISTRY, registry.previousRegistryDigest);
check("registry:artifact-laws", RECEIZ_V121_ARTIFACT_LAWS.length === 69 && RECEIZ_V121_ARTIFACT_LAWS.includes("receiz.identity.accepted-proof-full-session.v121"), String(RECEIZ_V121_ARTIFACT_LAWS.length));
...
check("skills:index", skillsIndex.schema === "receiz.ai-skills-index.v121" && skillsIndex.version === EXPECTED_VERSION, `${skillsIndex.schema}:${skillsIndex.version}`);
...
for (const entry of skillsIndex.skills ?? []) {
  ...
  check(`skill:${entry.name}`, manifest.schema === "receiz.ai-skill-contract.v121"
...
const attestation = json("receiz.migration.v120-v121.json");
...
check("migration:zero-change-preview", attestation.integrationPreviewChanges === 0 && attestation.integrationWritesPerformed === 0 && attestation.integrationPreviewDigest === null, `${attestation.integrationPreviewChanges}:${attestation.integrationWritesPerformed}:${attestation.integrationPreviewDigest}`);
...
const auditPath = "docs/releases/2026-08-19-v121-source-first-continuity-release.md";
...
console.log(JSON.stringify({
  schema: "receiz.app.v121.release-lock.v1",
  ok,
