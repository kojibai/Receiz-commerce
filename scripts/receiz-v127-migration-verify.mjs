...
import { reviewPublishedInventoryFindings, reviewPublishedInventoryActions } from './receiz-v127-inspection.mjs';
...
const MATRIX = 'eadd171a45fcc51e275a1c57de1eb8e67614757a5723d141793641edf7207a10';
const APP_REGISTRY = 'd1526fce85ba5e5cf259686ba0377d3c72b87040acc99a63a92eea6baf210315';
const RANGE = '>=127.0.0 <128.0.0';
const checks = [];
...
const attestation = json('receiz.migration.v126.0.0-v127.0.0.json');
...
check('application-version', pkg.version === '5.3.0' && attestation.applicationReleaseVersion === '5.3.0');
...
check('mcp-inventory', names.size === 330 && RECEIZ_MCP_TOOLS.length === 330 && RECEIZ_V124_MCP_TOOL_NAMES.length === 26 && [...RECEIZ_V123_MCP_TOOL_NAMES,...RECEIZ_V124_MCP_TOOL_NAMES,'receiz_material_url_open','receiz_sealed_kai_moment'].every(name => names.has(name)));
...
check('skills-identity', skills.schema === 'receiz.ai-skills-index.v127' && skills.version === VERSION && skills.rulesetVersion === VERSION && skills.registryDigest === REGISTRY && skills.operationMatrixDigest === MATRIX && skills.counts.skills === 43 && skills.counts.manifests === 37 && skills.counts.openaiAgentPrompts === 34);
...
console.log(JSON.stringify({schema:'receiz.repository.v127-migration-verification.v1',ok:checks.every(check=>check.ok),historyRewritten:false,packageVersion:VERSION,registryDigest:REGISTRY,operationMatrixDigest:MATRIX,checks},null,2));
