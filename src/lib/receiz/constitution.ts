...
if (!validation.ok) {
  throw new Error(`RECEIZ_V113_APP_REGISTRY_INVALID:${validation.issues.join(",")}`);
}
...
export async function verifyReceizAppConstitution(): Promise<Readonly<{
  ...
  return {
    ok: RECEIZ_APP_CONSTITUTION.previousRegistryDigest === RECEIZ_V113_REGISTRY_DIGEST && RECEIZ_APP_CONSTITUTION.version === RECEIZ_RULESET_VERSION,
...
export function createReceizAppCausalHistory(
  ...
  return createReceizCausalHistory({ ...options, registryDigest: RECEIZ_V113_REGISTRY_DIGEST });
}
