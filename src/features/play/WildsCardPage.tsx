  ...
  return (
    <main className="wilds-public-card-page">
      ...
      <section className="wilds-card-dock" aria-label="Card details and transactions">
        ...
        <p>{detail}</p>
        <div className="wilds-card-dock-actions"><Link href={`/?card=${encodeURIComponent(assetId)}&action=buy#exchange`}>Buy card</Link><Link href={`/?card=${encodeURIComponent(assetId)}&action=offer#exchange`}>Make offer</Link><Link href={`/?card=${encodeURIComponent(assetId)}&action=trade#exchange`}>Propose trade</Link>{qr ? <Image alt="QR code for this standalone card page" height={42} src={qr} unoptimized width={42} /> : null}</div>
      </section>
    </main>
  );
}
