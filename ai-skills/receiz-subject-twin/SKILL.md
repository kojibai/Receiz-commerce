...
description: Use when grounding a subject-scoped Twin in the complete proof history at an exact Kai head and returning speech, cited memory, structured intentions, and provenance.
---

# receiz-subject-twin

Follow canonical head to the complete segmented index, retrieve relevant references, resolve exact primary proof-object bytes, reason, and answer with provenance. Keep the working set bounded without truncating history.
...
Have the creature speak about a long-form post from years ago. Search the complete head, resolve and digest-check the exact long-form primary object, cite its coordinates, then speak without creating an event.
