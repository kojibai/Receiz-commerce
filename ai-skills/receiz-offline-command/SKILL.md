...
There is no current MCP queue mutation authority. Do not invent one and do not call retired proof-head, receipt, extraction, explanation, or Record-to-Seal helper tools. When replay produces an artifact, begin with `receiz_artifact_verify`; use the remaining four current artifact-transition tools only when current admission, verified actor/history, capability, staging, and named-domain commitment evidence are present.
