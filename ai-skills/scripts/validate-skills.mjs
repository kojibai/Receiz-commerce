...
function assertConstitutionalSkill(name) {
    ...
    if (manifest.schema !== "receiz.ai-skill-contract.v116" || manifest.name !== name || manifest.version !== "116.0.0") {
...
function assertOperationSkill(name) {
    ...
    if (nativeEvidence) {
        ...
        if (/identity\.getProfile|identity\.restoreAccount|identity\.appendAccountState|continuity\.reconcile|continuity\.commit|offline\.createCommandQueue|offline\.executeOrQueue|proofHead\.get|receipts\.verify|identityKeyId|expectedOwnershipHead|claimantKeyId|receiz_proof_head_get|receiz_receipt_verify|receiz_continuity_sync_plan|receiz_continuity_sync_execute|media\.publishIdentityImage/.test(text + serialized)) {
            fail(`${name} retains a retired obsolete-versioned prerequisite in the active v116 outcome`);
        }
...
function assertArtifactSkill(name) {
    ...
    if (manifest.schema !== "receiz.ai-skill-contract.v116" || manifest.name !== name || manifest.version !== "116.0.0") {
        fail(`${name}/manifest.json has invalid v116 schema, name, or version`);
    }
    if (requires?.ruleset !== "116.0.0" || requires.registryDigest !== artifactRegistryDigest || requires.operationMatrixDigest !== operationMatrixDigest) {
...
function assertGlobalReconciliationSkill() {
    ...
    const text = read(skillFile);
    const manifest = JSON.parse(read(manifestFile));
    if (manifest.schema !== "receiz.ai-skill-contract.v116" || manifest.version !== "116.0.0" || manifest.name !== name)
        fail(`${name}/manifest.json has invalid v116 identity`);
    for (const required of [artifactRegistryDigest, operationMatrixDigest, ">=116.0.0 <117.0.0"])
...
function assertCurrentManifest(name) {
    ...
    const manifest = JSON.parse(read(manifestFile));
    if (manifest.schema !== "receiz.ai-skill-contract.v116" || manifest.version !== "116.0.0" || manifest.name !== name)
        fail(`${name}/manifest.json is not current v116`);
    const requires = manifest.requires ?? {};
    if (requires.sdk !== ">=116.0.0 <117.0.0" || requires.mcp !== ">=116.0.0 <117.0.0"
        || requires.ruleset !== "116.0.0" || requires.registryDigest !== artifactRegistryDigest
